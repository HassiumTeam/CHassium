...
struct ast_node * block_node_init (struct vector_state * stmts) {
    ...
    state = (struct block_state *)malloc (sizeof (struct block_state));
    state->stmts = stmts;
...
struct ast_node * class_node_init (char * name, struct vector_state * extends, struct ast_node * body) {
    ...
    state = (struct class_state *)malloc (sizeof (struct class_state));
    state->name    = name;
...
struct ast_node * closure_node_init (struct vector_state * params, struct vector_state * return_type, struct ast_node * body) {
    ...
    state = (struct closure_state *)malloc (sizeof (struct closure_state));
    state->params      = params;
...
struct ast_node * expr_stmt_node_init (struct ast_node * expr) {
    ...
    state = (struct expr_stmt_state *)malloc (sizeof (struct expr_stmt_state));
    state->expr = expr;
...
struct ast_node * for_node_init (struct ast_node * pre_stmt, struct ast_node * expr, struct ast_node * rep_stmt, struct ast_node * body) {
    ...
    state = (struct for_state *)malloc (sizeof (struct for_state));
...
struct ast_node * foreach_node_init (char * id, struct ast_node * target, struct ast_node * body) {
    ...
    state = (struct foreach_state *)malloc (sizeof (struct foreach_state));
    state->id     = id;
    state->target = target;
    state->body   = body;
...
struct ast_node * func_call_node_init (struct ast_node * target, struct vector_state * args) {
    ...
    state = (struct func_call_state *)malloc (sizeof (struct func_call_state));
    state->target = target;
    state->args   = args;
...
struct ast_node * func_decl_node_init (char * id, struct vector_state * params, struct vector_state * return_type, struct ast_node * body) {
    ...
    state = (struct func_decl_state *)malloc (sizeof (struct func_decl_state));
    state->id          = id;
    state->params      = params;
    state->return_type = return_type;
    state->body        = body;
...
struct ast_node * id_node_init (char * id) {
    ...
    state = (struct id_state *)malloc (sizeof (struct id_state));
    state->id = id;
...
struct ast_node * if_node_init (struct ast_node * expr, struct ast_node * if_body, struct ast_node * else_body) {
    ...
    state = (struct if_state *)malloc (sizeof (struct if_state));
    state->expr      = expr;
...
struct ast_node * import_init (char * mod) {
    ...
    state = (struct import_state *)malloc (sizeof (struct import_state));
    state->mod = mod;
...
struct ast_node * list_decl_node_init (struct vector_state * elements) {
    ...
    state = (struct list_decl_state *)malloc (sizeof (struct list_decl_state));
...
struct ast_node * number_node_init (float f) {
    ...
    state = (struct number_state *)malloc (sizeof (struct number_state));
    state->f = f;
...
struct ast_node * obj_decl_node_init (struct vector_state * ids, struct vector_state * vals) {
    ...
    state = (struct obj_decl_state *)malloc (sizeof (struct obj_decl_state));
    state->ids  = ids;
    state->vals = vals;
...
struct ast_node * raise_node_init (struct ast_node * expr) {
    ...
    state = (struct raise_state *)malloc (sizeof (struct raise_state));
    state->expr = expr;
...
struct ast_node * return_node_init (struct ast_node * expr) {
    ...
    state = (struct return_state *)malloc (sizeof (struct return_state));
    state->expr = expr;
...
struct ast_node * string_node_init (char * str) {
    ...
    state = (struct string_state *)malloc (sizeof (struct string_state));
    state->str = str;
...
struct ast_node * subscript_node_init (struct ast_node * target, struct ast_node * index) {
    ...
    state = (struct subscript_state *)malloc (sizeof (struct subscript_state));
    state->target = target;
    state->index  = index;
...
struct ast_node * try_catch_node_init (struct ast_node * try_body, struct ast_node * catch_body) {
    ...
    state = (struct try_catch_state *)malloc (sizeof (struct try_catch_state));
...
struct ast_node * typeof_node_init (struct ast_node * expr) {
    ...
    state = (struct typeof_state *)malloc (sizeof (struct typeof_state));
    state->expr = expr;
...
struct ast_node * unary_op_node_init (unary_op_type_t type, struct ast_node * target) {
    ...
    state = (struct unary_op_state *)malloc (sizeof (struct unary_op_state));
    state->type = type;
    state->target = target;
...
struct ast_node * use_node_init (struct vector_state * imports, char * mod) {
    ...
    state = (struct use_state *)malloc (sizeof (struct use_state));
...
struct ast_node * while_node_init (struct ast_node * expr, struct ast_node * body) {
    ...
    state = (struct while_state *)malloc (sizeof (struct while_state));
    state->expr = expr;
    state->body = body;
