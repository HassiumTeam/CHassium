...
static struct ast_node *parse_assign(struct parser *parser)
{
    ...
    else if (matchtok(parser, TOK_ASSIGN))
    {
        ...
        return bin_op_node_new(BIN_OP_ASSIGN, left, bin_op_node_new(op_type, left, parse_assign(parser)));
    }
    return left;
}
...
static struct tok *expecttokv(struct parser *parser, toktype_t type, char *val)
{
    if (matchtokv(parser, type, val))
    {
        struct tok *ret = curtok(parser);
