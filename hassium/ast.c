...
{
    struct attrib_node *inner = (struct attrib_node *)calloc(1, sizeof(struct attrib_node));
    inner->target = target;
    inner->attrib = attrib;
    return ast_node_new(ATTRIB_NODE, inner);
}

struct ast_node *bin_op_node_new(bin_op_type_t type, struct ast_node *left, struct ast_node *right)
{
    struct bin_op_node *inner = (struct bin_op_node *)calloc(1, sizeof(struct bin_op_node));
    inner->type = type;
    inner->left = left;
    inner->right = right;
...
struct ast_node *class_decl_node_new(char *name, struct ast_node *extends, struct ast_node *body)
{
    struct class_decl_node *inner = (struct class_decl_node *)calloc(1, sizeof(struct class_decl_node));
    inner->name = name;
...
{
    struct code_block_node *inner = (struct code_block_node *)calloc(1, sizeof(struct code_block_node));
...
{
    struct expr_stmt_node *inner = (struct expr_stmt_node *)calloc(1, sizeof(struct expr_stmt_node));
    inner->expr = expr;
    return ast_node_new(EXPR_STMT_NODE, inner);
}

struct ast_node *for_node_new(struct ast_node *initial, struct ast_node *condition, struct ast_node *repeated, struct ast_node *body)
{
    struct for_node *inner = (struct for_node *)calloc(1, sizeof(struct for_node));
...
struct ast_node *foreach_node_new(char *id, struct ast_node *target, struct ast_node *body)
{
    struct foreach_node *inner = (struct foreach_node *)calloc(1, sizeof(struct foreach_node));
    inner->id = id;
    inner->target = target;
    inner->body = body;
...
struct ast_node *func_decl_node_new(char *name, struct vec *params, struct ast_node *ret_type, struct ast_node *body)
{
    struct func_decl_node *inner = (struct func_decl_node *)calloc(1, sizeof(struct func_decl_node));
    inner->name = name;
    inner->params = params;
...
struct ast_node *if_node_new(struct ast_node *predicate, struct ast_node *body, struct ast_node *else_body)
{
...
{
    struct import_node *inner = (struct import_node *)calloc(1, sizeof(struct import_node));
    inner->target = target;
...
{
    struct invoke_node *inner = (struct invoke_node *)calloc(1, sizeof(struct invoke_node));
    inner->target = target;
    inner->args = args;
...
{
    struct num_node *inner = (struct num_node *)calloc(1, sizeof(struct num_node));
    inner->value = value;
...
{
    struct obj_decl_node *inner = (struct obj_decl_node *)calloc(1, sizeof(struct obj_decl_node));
    inner->keys = keys;
    inner->values = values;
...
{
    struct raise_node *inner = (struct raise_node *)calloc(1, sizeof(struct raise_node));
    inner->value = value;
...
{
    struct return_node *inner = (struct return_node *)calloc(1, sizeof(struct return_node));
    inner->value = value;
...
{
    struct string_node *inner = (struct string_node *)calloc(1, sizeof(struct string_node));
    inner->value = value;
    return ast_node_new(STRING_NODE, inner);
}

struct ast_node *subscript_node_new(struct ast_node *target, struct ast_node *key)
{
    struct subscript_node *inner = (struct subscript_node *)calloc(1, sizeof(struct subscript_node));
    inner->target = target;
    inner->key = key;
...
{
    struct super_node *inner = (struct super_node *)calloc(1, sizeof(struct super_node));
    inner->args = args;
    return ast_node_new(SUPER_NODE, inner);
}

struct ast_node *try_catch_node_new(struct ast_node *try, struct ast_node *catch, char *id)
{
    struct try_catch_node *inner = (struct try_catch_node *)calloc(1, sizeof(struct try_catch_node));
    inner->try = try;
    inner->catch = catch;
    inner->id = id;
...
struct ast_node *unary_op_node_new(unary_op_type_t type, struct ast_node *target)
{
    struct unary_op_node *inner = (struct unary_op_node *)calloc(1, sizeof(struct unary_op_node));
    inner->type = type;
    inner->target = target;
    return ast_node_new(UNARY_OP_NODE, inner);
}

struct ast_node *while_node_new(struct ast_node *condition, struct ast_node *body)
{
    struct while_node *inner = (struct while_node *)calloc(1, sizeof(struct while_node));
    inner->condition = condition;
    inner->body = body;
...
static void string_node_free(struct string_node *node)
{
    free(node->value);
}
